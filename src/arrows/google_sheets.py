...
def arrow_to_googlesheet(
    ...
    sheet.from_arrow(
        arrow=arrow, sheet_range=sheet_range, overwrite_sheet=overwrite_sheet, overwrite_range=overwrite_range
    )
    return sheet
...
class SpreadSheet:
    ...
    def share(self, email, role='reader', type='user', send_notification=True):
        ...
        for each_email in email:
            ...
            drive_service.permissions().create(
                fileId=self.spreadsheet_id, body=permission, sendNotificationEmail=send_notification
            ).execute()
    ...
    def sheets(self):
        ...
        sheets = [
            Sheet(spreadsheet_id=self, sheet_name=each['properties']['title'], sheet_id=each['properties']['sheetId'])
            for each in sheets
        ]
        return sheets
    ...
    def rename(self, spreadsheet_name):
        ...
        drive_service.files().update(
            fileId=self.spreadsheet_id, body={'name': spreadsheet_name}, fields='id, name'
        ).execute()
...
class Sheet:
    ...
    def url(self):
        if self.sheet_id is None:
            self.get_sheet_id()
        return (
            f'https://docs.google.com/spreadsheets/d/{self.spreadsheet_id}/edit?gid={self.sheet_id}#gid={self.sheet_id}'
        )
        ...
        requests = [
            {
                'updateSheetProperties': {
                    'properties': {'sheetId': self.sheet_id, 'title': sheet_name},
                    'fields': 'title',
                }
            }
        ]
    ...
    def delete(self):
        ...
        _sheets_service = _sheets_api()
        requests = [{'deleteSheet': {'sheetId': self.sheet_id}}]
        body = {'requests': requests}
        _sheets_service.spreadsheets().batchUpdate(spreadsheetId=self.spreadsheet_id, body=body).execute()
